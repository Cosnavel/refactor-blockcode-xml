...
function SiteChecker(options, handlers) {
    ...
    this.sitePagesChecked = new UrlCache({
        ...
        },
        {
            item: function (input, done) {
                ...
                if (options.honorRobotExclusions === true) {
                    getRobotsTxt(thisObj.currentSiteUrl, options).then(
                        function (robots) {
                            thisObj.currentRobotsTxt = robots

                            maybeCallback(thisObj.handlers.robots)(
                                robots,
                                thisObj.currentCustomData,
                            )
                            /*})
    ...
    this.htmlUrlChecker = new HtmlUrlChecker(this.options, {
        html: function (tree, robots, response, pageUrl, customData) {
            ...
            if (response.url !== pageUrl) {
                ...
                for (var i = 0; i < response.redirects.length; i++) {
                    ...
                    thisObj.sitePagesChecked.set(
                        response.redirects[i].url,
                        true,
                    )
                }
            }

            maybeCallback(thisObj.handlers.html)(
                tree,
                robots,
                response,
                pageUrl,
        ...
        page: function (error, pageUrl, customData) {
            ...
            maybeCallback(thisObj.handlers.site)(
                thisObj.currentPageError,
                thisObj.currentSiteUrl,
                thisObj.currentCustomData,
            )
...
function isAllowed(instance, link) {
    if (
        instance.options.honorRobotExclusions ===
        true /*&& instance.currentRobotsTxt!=null*/
    ) {
        // TODO :: remove condition when/if `linkObj.invalidate()` is used in `HtmlChecker`
        if (link.url.resolved !== null) {
            return instance.currentRobotsTxt.isAllowed(
                instance.options.userAgent,
                link.url.parsed.pathname,
            )
        }
    }
...
function maybeEnqueuePage(instance, link, customData) {
    ...
    tagGroup =
        instance.options.tags.recursive[instance.options.filterLevel][
            link.html.tagName
        ]
    ...
    if (
        link.url.redirected != null &&
        instance.sitePagesChecked.get(link.url.redirected) === true
    ) {
