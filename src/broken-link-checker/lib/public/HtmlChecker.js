    ...
    if (this.active === false) {
        ...
        parseHtml(html)
            ...
            .then(function (links) {
                ...
                if (
                    thisObj.urlChecker.numActiveLinks() === 0 &&
                    thisObj.urlChecker.numQueuedLinks() === 0
                ) {
                    complete(thisObj)
                }
            })
...
function enqueueLink(link, instance) {
    ...
    if (instance.linkEnqueued instanceof Error) {
        ...
        link.brokenReason =
            instance.linkEnqueued.message === 'Invalid URI'
                ? 'BLC_INVALID'
                : 'BLC_UNKNOWN'
...
function excludeLink(link, instance) {
    ...
    if (
        instance.options.excludeExternalLinks === true &&
        link.internal === false
    )
        return 'BLC_EXTERNAL'
    if (
    ...
    if (
        ...
        return 'BLC_SAMEPAGE'
    if (
        instance.options.excludedSchemes[
            link.url.parsed.extra.protocolTruncated
        ] === true
    )
        return 'BLC_SCHEME'
    ...
    if (instance.options.honorRobotExclusions === true) {
        ...
        if (
            link.html.attrs != null &&
            link.html.attrs.rel != null &&
            linkTypes(link.html.attrs.rel).nofollow === true
        ) {
            return 'BLC_ROBOTS'
        }
    }
