...
function HtmlUrlChecker(options, handlers) {
            ...
            item: function (input, done) {
                ...
                streamHtml(
                    ...
                    .then(function (result) {
                        ...
                        thisObj.currentRobots = new RobotDirectives({
                            userAgent: thisObj.options.userAgent,
                        })

                        robotHeaders(thisObj)

                        // Passes robots instance so that headers are included in robot exclusion checks
                        thisObj.htmlChecker.scan(
                            result.stream,
                            result.response.url,
                            thisObj.currentRobots,
                        )
                    })
    ...
    this.htmlChecker = new HtmlChecker(this.options, {
        ...
        junk: function (result) {
            maybeCallback(thisObj.handlers.junk)(
                result,
                thisObj.currentCustomData,
            )
        },
        link: function (result) {
            maybeCallback(thisObj.handlers.link)(
                result,
                thisObj.currentCustomData,
            )
        },
...
function completedPage(instance, error) {
    maybeCallback(instance.handlers.page)(
        error,
        instance.currentPageUrl,
        instance.currentCustomData,
    )
...
function robotHeaders(instance) {
    ...
    if (instance.currentResponse.headers['x-robots-tag'] != null) {
        instance.currentRobots.header(
            instance.currentResponse.headers['x-robots-tag'],
        )
    }
}
