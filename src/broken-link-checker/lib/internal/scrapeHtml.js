            ...
            for (i = 0; i < numAttrs; i++) {
                ...
                if (linkAttrs[attrName] === true) {
                    ...
                    if (node.nodeName === 'meta' && attrName === 'content') {
                        if (
                            node.attrMap['http-equiv'] != null &&
                            node.attrMap['http-equiv'].toLowerCase() ===
                                'refresh'
                        ) {
    ...
    walk(rootNode, function (node) {
        switch (node.nodeName) {
            ...
            case 'meta': {
                if (
                    find.robots === true &&
                    node.attrMap.name != null &&
                    node.attrMap.content != null
                ) {
                    ...
                    switch (name) {
                        ...
                        default: {
                            if (
                                name === 'robots' ||
                                RobotDirectives.isBot(name) === true
                            ) {
