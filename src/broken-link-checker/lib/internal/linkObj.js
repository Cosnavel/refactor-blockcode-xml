...
linkObj.relation = function (link, url_parsed) {
    ...
    if (url_parsed.protocol === null || link.base.parsed.protocol === null) {
        ...
        link.samePage =
            link.internal === true && relation >= urlobj.component.PATH
    }
...
linkObj.resolve = function (link, base, options) {
    ...
    // TODO :: options.clone=true
    var resolvedUrl_parsed = urlobj.resolve(
        resolvedBase_parsed,
    ...
    if (
        options.acceptedSchemes[resolvedUrl_parsed.extra.protocolTruncated] ===
        true
    ) {
    ...
    // Else could not be properly resolved
        ...
        if (resolvedUrl_parsed.extra.type === urlobj.type.ABSOLUTE) {
            ...
            if (
                options.acceptedSchemes[base_parsed.extra.protocolTruncated] ===
                true
            ) {
                link.internal = false
                link.samePage = false
